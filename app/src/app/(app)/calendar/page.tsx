...
export default function CalendarPage() {
  ...
  async function handleSearch(e?: React.FormEvent) {
    ...
    // Clamp fromDate to today if the month includes past dates
    ...
    // Don't search if the entire month is in the past
    if (toDate < today) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-5 text-xs text-slate-400">
            <span className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /> Cheap
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-full bg-amber-500" /> Mid-range
            </span>
            <span className="flex items-center gap-1.5">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    `}
                  >
                    <span className={`text-[10px] sm:text-xs ${dp ? "text-slate-300" : "text-slate-600"}`}>
                      {day}
                    </span>
                    {dp && (
          ...
          {selectedDp && (
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/10 p-4 flex items-center justify-between">
              <div>
                <div className="text-xs text-blue-400/70 font-medium">Selected Date</div>
